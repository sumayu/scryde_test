                          ...
                          <div className="grid grid-cols-2 gap-4 w-full max-md:grid-cols-1 max-w-md">
                            {branch.paths.map((path: any, pIdx: number) => (
                              <div key={pIdx} className="flex flex-col items-center space-y-4">
                                ...
                                {/* Третья и четвёртая профессии — по каждому ответвлению */}
                                <div className="flex flex-col gap-6 w-full">
                                  {(path.secondPaths || []).map((sp: any, spIdx: number) => (
                                    <div key={spIdx} className="flex flex-col items-center space-y-4">
                                      <button 
                                        onClick={() => {
                                          setSelectedClass(sp.third);
                                          setTimeout(() => {
                                            document.getElementById('class-details')?.scrollIntoView({ behavior: 'smooth' });
                                          }, 100);
                                        }}
                                        className={`w-full group/btn relative flex flex-col items-center p-3 border transition-all rounded-sm hover:scale-105 overflow-hidden shadow-[0_0_15px_rgba(37,99,235,0.1)] ${selectedClass === sp.third ? 'border-blue-400 bg-blue-700 shadow-[0_0_20px_rgba(37,99,235,0.4)]' : 'border-blue-900/50 bg-slate-900/40 hover:border-blue-400 hover:bg-blue-900/20 hover:shadow-[0_0_12px_rgba(59,130,246,0.3)]'}`}
                                      >
                                        ...
                                        <div className="w-14 h-14 mb-2 relative z-10 flex items-center justify-center">
                                          ...
                                          <img src={sp.thirdIcon} alt={sp.third} referrerPolicy="no-referrer" className="w-10 h-10 object-contain relative z-20 group-hover/btn:scale-110 transition-transform drop-shadow-[0_0_8px_rgba(37,99,235,0.5)]" />
                                        </div>
                                        <span className={`relative z-10 font-display text-[9px] tracking-widest text-center leading-tight transition-colors ${selectedClass === sp.third ? 'text-white' : 'text-blue-300 group-hover/btn:text-white'}`}>
                                          {sp.third}
                                        </span>
                                      </button>
                                      ...
                                      {sp.fourth && (
                                        <>
                                          ...
                                          <button 
                                            onClick={() => {
                                              setSelectedClass(sp.fourth);
                                              setTimeout(() => {
                                                document.getElementById('class-details')?.scrollIntoView({ behavior: 'smooth' });
                                              }, 100);
                                            }}
                                            className={`w-full group/btn relative flex flex-col items-center p-3 border transition-all rounded-sm hover:scale-105 overflow-hidden shadow-[0_0_15px_rgba(250,204,21,0.15)] ${selectedClass === sp.fourth ? 'border-yellow-400 bg-yellow-900/40 shadow-[0_0_20px_rgba(250,204,21,0.4)]' : 'border-yellow-700/40 bg-slate-900/40 hover:border-yellow-400 hover:bg-yellow-900/10 hover:shadow-[0_0_12px_rgba(250,204,21,0.3)]'}`}
                                          >
                                            <div className="absolute inset-0 bg-yellow-500/10 opacity-0 group-hover/btn:opacity-100 transition-opacity" />
                                            <div className="w-14 h-14 mb-2 relative z-10 flex items-center justify-center">
                                              <div className="absolute inset-0 bg-yellow-900/30 rounded-md border border-yellow-500/30 shadow-[0_0_10px_rgba(250,204,21,0.1)] group-hover/btn:border-yellow-400 transition-colors pointer-events-none" />
                                              <img src={sp.fourthIcon} alt={sp.fourth} referrerPolicy="no-referrer" className="w-10 h-10 object-contain relative z-20 group-hover/btn:scale-110 transition-transform drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]" />
                                            </div>
                                            <span className={`relative z-10 font-display text-[9px] tracking-widest text-center leading-tight transition-colors ${selectedClass === sp.fourth ? 'text-white' : 'text-yellow-300 group-hover/btn:text-white'}`}>
                                              {sp.fourth}
                                            </span>
                                          </button>
                                        </>
